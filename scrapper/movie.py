...
def scrape_from(url):
    ...
    ul = soup.find("ul",class_="ipc-inline-list ipc-inline-list--show-dividers sc-d8941411-2 cdJsTz baseAlt")
    ...
    directors = ', '.join(a.text for a in directors_element.find_all("a", class_="ipc-metadata-list-item__list-content-item ipc-metadata-list-item__list-content-item--link")) if directors_element else ''

    writers_element = all_li[1] if len(all_li) > 1 else None
    writers = ', '.join(a.text for a in writers_element.find_all("a", class_="ipc-metadata-list-item__list-content-item ipc-metadata-list-item__list-content-item--link")) if writers_element else ''

    stars_element = all_li[2] if len(all_li) > 2 else None
    stars = ', '.join(a.text for a in stars_element.find_all("a", class_="ipc-metadata-list-item__list-content-item ipc-metadata-list-item__list-content-item--link")) if stars_element else ''
